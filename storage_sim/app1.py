...
def create_system():
    ...
    try:
        ...
        system = System(id=system_id, name=data.get("name", f"Storage System {PORT}"))
...
def update_settings(settings_id):
    ...
    data = request.get_json(silent=True) or {}
    try:
        settings["snapshot_frequency"] = data.get("snapshot_frequency", settings.get("snapshot_frequency", "daily"))
